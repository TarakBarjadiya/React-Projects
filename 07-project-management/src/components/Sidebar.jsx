...
let liClass = "w-full text-left cursor-pointer px-2 py-1 rounded-sm my-1 truncate hover:text-stone-600 hover:bg-stone-800 hover:tracking-wider";

export default function Sidebar({ onStartCreateNew, projectData, onProjectSelect, selectedId }) {
        ...
        {(projectData.length == 0) ? (
          <>
            <div className="container flex h-2/4 justify-center items-center">
              <p className="text-neutral-200 font-medium m">You don't have any projects yet.</p>
            </div>
          </>
        )
          :
          <ul className={ulClass}>
            {projectData.map((project) => {
              ...
              return (
                <li onClick={() => onProjectSelect(project.id)} key={project.id} className={liClass} >
